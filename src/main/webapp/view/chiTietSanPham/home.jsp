...
<div class="gap-element clearfix" style="display:block; height:auto; padding-top:30px"></div>
<button type="submit"><a href="/chi-tiet-san-pham/view-add">Thêm chi tiết sản phẩm</a></button>
...
<table class="table table-striped">
    ...
    <tbody>
    <c:forEach items="${list}" var="ctsp" varStatus="i">
        <tr>
            <td>${i.index + 1}</td>
            <td>${ctsp.sanPham.tenSp}</td>
            <td>${ctsp.nhaSX.tenNSX}</td>
            <td>${ctsp.mauSac.tenMau}</td>
            <td>${ctsp.dongSP.tenDSP}</td>
            <td>${ctsp.namBH}</td>
            ...
            <td>${ctsp.moTa}</td>
            <th>
                <button><a href="/chi-tiet-san-pham/detail?id=${ctsp.id}">Detail</a></button>
                <button><a href="/chi-tiet-san-pham/view-update?id=${ctsp.id}">Edit</a></button>
                <button onclick="return confirm('Bạn chắc chắn muốn xóa')"><a
                        href="/chi-tiet-san-pham/remote?id=${ctsp.id}">Remove</a></button>
            </th>
        </tr>
    </c:forEach>
    </tbody>
