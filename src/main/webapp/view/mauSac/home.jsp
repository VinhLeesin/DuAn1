...
<div class="gap-element clearfix" style="display:block; height:auto; padding-top:30px"></div>
<button type="submit"><a href="/mau-sac/view-add">Thêm màu sắc</a></button>
...
<table class="table table-striped">
    ...
    <c:forEach items="${list}" var="ms" varStatus="i">
        <tr>
            ...
            <td>${ms.tenMau}</td>
            <td>
                <button><a href="/mau-sac/detail?id=${ms.id}">Detail</a></button>
                <button><a href="/mau-sac/view-update?id=${ms.id}">Edit</a></button>
                <button onclick="return confirm('Bạn chắc chắn muốn xóa')"><a
                        href="/mau-sac/remote?id=${ms.id}">Remove</a></button>
            </td>
        </tr>
    </c:forEach>
    </tbody>
