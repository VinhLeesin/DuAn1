...
<nav class="navbar navbar-expand-sm bg-light navbar-light">
    ...
    <div class="form-inline" style="margin-top: 20px; margin-left: 100px;">
        <form action="/cua-hang/search" method="get">
...
<div class="gap-element clearfix" style="display:block; height:auto; padding-top:30px"></div>
<button type="submit"><a href="/cua-hang/view-add">Thêm cửa hàng</a></button>
...
<table class="table table-striped">
    ...
    <tbody>
    <c:forEach items="${list}" var="ch" varStatus="i">
        <tr>
            <td>${i.index + 1 }</td>
            <td>${ch.maCH}</td>
            <td>${ch.tenCH}</td>
            ...
            <td>${ch.quocGia}</td>
            <td>
                <button><a href="/cua-hang/detail?id=${ch.id}">Detail</a></button>
                <button><a href="/cua-hang/view-update?id=${ch.id}">Edit</a></button>
                <button onclick="return confirm('Bạn chắc chắn muốn xóa')"><a
                        href="/cua-hang/remote?id=${ch.id}">Remove</a></button>
            </td>
        </tr>
    </c:forEach>
    </tbody>
