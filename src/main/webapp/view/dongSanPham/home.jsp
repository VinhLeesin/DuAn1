...
<nav class="navbar navbar-expand-sm bg-light navbar-light">
    ...
    <div class="form-inline" style="margin-top: 20px; margin-left: 100px;">
        <form action="/dong-san-pham/search" method="get">
...
<div class="gap-element clearfix" style="display:block; height:auto; padding-top:30px"></div>
<button type="submit"><a href="/dong-san-pham/view-add">Thêm dòng sản phẩm</a></button>
...
<table class="table table-striped">
    ...
    <tbody>
    <c:forEach items="${list}" var="dsp" varStatus="i">
        <tr>
            <td>${i.index + 1}</td>
            <td>${dsp.maDSP}</td>
            <td>${dsp.tenDSP}</td>
            <td>
                <button><a href="/dong-san-pham/detail?id=${dsp.id}">Detail</a></button>
                <button><a href="/dong-san-pham/view-update?id=${dsp.id}">Edit</a></button>
                <button onclick="return confirm('Bạn chắc chắn muốn xóa')"><a href="/dong-san-pham/remote?id=${dsp.id}">Remove</a>
                </button>
            </td>
        </tr>
    </c:forEach>
